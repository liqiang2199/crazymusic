...
//      return compress(bitmap, maxSize); // 这里再进行质量压缩的意义不大，反而耗资源，删除
    ...
    public static void ratioAndGenThumb(String imgPath, String outPath, float pixelW, boolean needsDelete) throws FileNotFoundException {
        Bitmap bitmap = ratio(imgPath, pixelW, pixelW);
        storeImage(bitmap, outPath);
        // Delete original file
        if (needsDelete) {
            File file = new File(imgPath);
